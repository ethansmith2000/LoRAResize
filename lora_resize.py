...
def change_lora_rank(state_dict,
    ...
    for key in tqdm(weights_with_lora):
        ...
        new_lora_up, new_lora_down = get_least_squares_solution(full_mat, lora_up, lora_down, rank=rank, steps=steps,
...
def simple_expand_lora(state_dict, new_rank, down_name="lora_down", up_name="lora_up",):
    ...
    assert new_rank > rank, "New rank must be greater than the old rank"
    ...
    for key in tqdm(weights_with_lora):
        ...
        new_lora_up = torch.zeros((lora_up.shape[0], new_rank), device=lora_up.device)
        new_lora_down = torch.zeros((new_rank, lora_down.shape[1]), device=lora_down.device)
